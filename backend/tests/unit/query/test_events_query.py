...
def test_source_filter_in_clause():
    sid = uuid.uuid4()
    stmt = build_events_query(EventsQueryParams(source=[sid]), role=None)
    sql = _sql(stmt)
    assert "events.source_id in" in sql


def test_source_type_subquery():
    stmt = build_events_query(EventsQueryParams(source_type=["rss", "taxii"]), role=None)
    sql = _sql(stmt)
...
def test_observed_from_to_bounds():
    ...
    stmt = build_events_query(EventsQueryParams(observed_from=t1, observed_to=t2), role=None)
    sql = _sql(stmt)
...
def test_tlp_name_to_uuid_subquery():
    stmt = build_events_query(EventsQueryParams(tlp=["clear", "green"]), role=None)
    sql = _sql(stmt)
...
def test_attack_technique_filter_subquery():
    stmt = build_events_query(EventsQueryParams(attack_technique=["T1190"]), role=None)
    sql = _sql(stmt)
...
def test_tag_and_semantics_coalesce_null():
    stmt = build_events_query(EventsQueryParams(tag=["apt28", "phishing"]), role=None)
    sql = _sql(stmt)
    assert "coalesce(events.tags" in sql  # Pitfall 6 — NULL-safe
    assert "@>" in sql


def test_include_archived_default_false():
    stmt = build_events_query(EventsQueryParams(), role=None)
    sql = _sql(stmt)
    assert "events.archived =" in sql


def test_include_archived_true_disables_filter():
    stmt = build_events_query(EventsQueryParams(include_archived=True), role=None)
    sql = _sql(stmt)
