...
D-28: max_retries=0 — we own the retry policy inline (30/60/120s with
cursor-advance semantics). Dramatiq middleware retry would re-queue the
whole tick and potentially double-dispatch.

Pitfall 2: the actor body is wrapped in run_dispatch_tick()'s broad
...
def webhook_dispatch_tick(_token: str = "tick") -> None:
    """Fired every 60s by APScheduler. D-04."""
