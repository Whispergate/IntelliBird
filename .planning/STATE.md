...
status: verifying
stopped_at: Completed 34-02-PLAN.md — Wave 1 browser extension core files
last_updated: "2026-05-06T14:17:16.146Z"
...
**Current focus:** v4.0 Phase 34 — Browser Extension (in progress, plan 03/3 complete)

## Current Position

Milestone: v4.0 Threat Intelligence Platform Maturity
Phase: 34 — Browser Extension (in progress — 3/3 plans shipped)
Plan: 03/03 complete — Global /iocs Next.js page (RSC + IOCsGlobalClient) + TopNav IOCs link. EXT-02 fulfilled. Both Wave 0 vitest tests (EXT-02a/EXT-02b) green.
...
| Phase 29-sigma-rule-engine P05 | 8 | 2 tasks | 3 files |
...
| Phase 30-notification-channels P06 | 3 | 1 tasks | 1 files |
| Phase 30-notification-channels P07 | 18 | 2 tasks | 4 files |
| Phase 30-notification-channels P08 | 6 | 1 tasks | 1 files |
| Phase 31-case-management P01 | 2 | 2 tasks | 2 files |
...
- [Phase 26-taxii-outbound-server]: TaxiiEnvelope overrides model_dump() to omit 'next' key when more=False per TAXII 2.1 spec
...
- [Phase 29-sigma-rule-engine]: events_query import path is app.services.events_query (NOT app.db.events_query)
- [Phase 29-sigma-rule-engine]: Admin test endpoint builds event_dict inline (async) instead of reusing sync _build_event_dict
- [Phase 29-sigma-rule-engine]: Level field uses Select with fixed Sigma level enum (not free-text Input)
- [Phase 29-sigma-rule-engine]: Test-against-corpus pattern: inline Test button in add dialog shows X/100 events matched badge
- [Phase 30-notification-channels]: Use except Exception (not except ImportError) for module-level import guards in stub tests — webhook_dispatcher pydantic ValidationError is not ImportError
...
- [Phase 30-notification-channels]: Port 2586 for ntfy to avoid clash with common ports; no network isolation needed (clearnet push, unlike tor darkweb_net); ntfy-cache + ntfy-etc volumes for persistence
- [Phase 30-07]: Email/PagerDuty/Opsgenie/ntfy credentials serialised as JSON in bearer token field for auth_enc shaping; api-client.generated.ts destination_type hand-widened to 8 types
- [Phase 30-notification-channels]: Use as-unknown-as-WebhookAuth cast for opaque credential dicts in webhookSchema.ts — avoids generated api-client.ts modification while correctly encoding flat dicts for backend dispatcher
- [Phase 31-case-management]: Wave-0 TDD scaffold: pytest.skip stubs define case API contract before router ships; test_case_isolation in leakage file for cross-project isolation coverage (CASE-05)
- [Phase 31-case-management]: Poll-based case summary delivery (no SSE); token budget from app.services.llm.token_budget; get_redis() not get_redis_client
- [Phase 31-case-management]: 032_cases migration uses TEXT for status/severity (not ENUM) to match ORM model; adds search_path reset to fix ag_catalog table creation bug from 029 side-effect
...
- [Phase 34]: Triple-guard pattern for testable MV3 service workers: typeof importScripts, typeof module, and typeof browser guards let background.js be required() in Node.js tests without error
- [Phase 34]: background.test.js converted from ESM import to CJS require() to match module.exports guard in background.js (classic service workers cannot use export keyword)
...
Last session: 2026-05-06T14:17:11.363Z
Stopped at: Completed 34-02-PLAN.md — Wave 1 browser extension core files
Resume file: None
